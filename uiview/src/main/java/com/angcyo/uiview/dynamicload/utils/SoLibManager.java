...
package com.angcyo.uiview.dynamicload.utils;
...
public final class SoLibManager {
        ...
        try {
            ...
            while (entries.hasMoreElements()) {
                ...
                if (zipEntryName.endsWith(".so") && zipEntryName.contains(cpuArchitect)) {
                    ...
                    //mSoExecutor.execute(new CopySoTask(context, zipFile, zipEntry, lastModify));//为了有效关闭zipFile文件, 不采用线程池,

                    //外部调用的时候, 请确保在线程中执行
                    CopySoTask copySoTask = new CopySoTask(context, zipFile, zipEntry, lastModify);
                    copySoTask.run();
                }
            }
        } catch (Exception e) {
    ...
    private class CopySoTask implements Runnable {
        ...
        private void writeSoFile2LibDir() throws IOException {
            ...
            copy(is, fos);
            //mZipFile.close();//fix 多个so拷贝, 被中断关闭的bug
        }
